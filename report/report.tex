...
\begin{figure}[htbp]
    \centering
    \subfloat[Polynomial fit]{\includegraphics[width=0.49\textwidth]{images/noise_2_0_polynomial.png}\label{fig:f1}}
    \hfill
...
On the left, (a) is a polynomial fit that is too complex a 
function type for the data points. 
It would not predict future $y$-values for a set of 
given $x$-values as successfully as the linear fit on 
the right (b).
...
estimation could be directly computed as follows:
