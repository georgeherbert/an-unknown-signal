...
be non-linear.
Then, I created a program, \texttt{order.py}, that calculated
the cross-validation error for each of these line segments
when trained using a model from a polynomial of degree 2,
to a polynomial of degree 10. A small section of the output
from this program is shown in Table \ref{tab:order.py}.

\begin{table}
\begin{center}
    \caption{\label{tab:order.py}Section of the output from \texttt{order.py}}
\begin{tabular}{l l l l} 
    \hline\hline
    Filename & Line segment & Polynomial order & Cross-validation error \\ [0.5ex] 
    \hline
...
Having analysed this output, it was clear that the
polynomial degree that most consistely gave a low
cross-validation error was a polynomial of degree 3
(i.e. a cubic).
